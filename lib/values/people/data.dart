...
const Person ALEKSANDER_GROSZAN = Person(
    name: 'Aleksander Groszan',
    druzyna: '25. RDHS „Kotwicz”',
    rankHarc: RankHarc.zhpCwik,
    org: Org.zhp,
    email: ["olekgroszan@gmail.com"]
);
...
const Person ALEKSANDRA_KOZUBAL = Person(
...
const Person ARTUR_RUSA = Person(
  ...
  hufiec: huf_zhp_pulawy,
  druzyna: '113. SDSH',
  org: Org.zhp,
);
const Person BARBARA_THOMAS = Person(
    name: 'Barbara Thomas',
    rankInstr: RankInstr.phm
);
const Person BARTLOMIEJ_DANIELAK = Person(
...
const Person BARTOSZ_DABROWSKI = Person(
    name: 'Bartosz Dąbrowski',
    rankHarc: RankHarc.zhpWywiadowca,
    org: Org.zhp
);
const Person BARTOSZ_IGNASIAK = Person(
...
const Person CELINA_STANISLAWSKA = Person(
...
const Person DANIEL_KORZEB = Person(
  name: 'Daniel Korzeb',
  rankHarc: RankHarc.zhpOdkrywca,
...
const Person DAWID_PIOTR_JACHIMOWSKI = Person(
...
const Person DAWID_WYSZYNSKI = Person(
...
const Person FILIP_KWIATKOWSKI = Person(
...
const Person FILIP_SOCHAJ = Person(
    name: 'Filip Sochaj'
);
const Person FILIP_SWIDEREK = Person(
    name: 'Filip Świderek'
);
...
const Person HANNA_KESKA = Person(
    name: 'Hanna Kęska',
    rankHarc: RankHarc.zhpSamarytanka,
    org: Org.zhp,
...
const Person HUBERT_NAPIERALA = Person(
...
const Person JAKUB_BRYLA = Person(
    name: 'Jakub Bryła',
...
const Person JAN_KWIATKOWSKI = Person(
...
const Person JOANNA_ZUBEK = Person(
    ...
    hufiec: 'Hufiec ZHP Warszawa Mokotów',
    rankInstr: RankInstr.pwd,
    rankHarc: RankHarc.zhpHOd,
    org: Org.zhp,
    email: ['joanna.zubek@zhp.net.pl']
);
...
const Person JULIA_BOLOZ = Person(
    name: 'Julia Bołoz',
...
const Person JULIA_WIERZBA = Person(
    name: 'Julia Wierzba',
...
const Person KAROL_MALINSKI = Person(
    name: 'Karol Maliński',
...
const Person KAROL_MALUS = Person(
    name: 'Karol Malus',
    rankHarc: RankHarc.dhc
);
...
const Person KATARZYNA_MAZUR = Person(
...
const Person KINGA_ZEBRACKA = Person(
    name: 'Kinga Żebracka',
...
const Person KLAUDIUSZ_PALUCH = Person(
    name: 'Klaudiusz Paluch'
);
const Person KLEMENTYNA_MARWICZ = Person(
...
const Person KRZYSZTOF_KRAWCZYK = Person(
...
const Person KRZYSZTOF_RODZINKA = Person(
...
const Person LUCJA_PRABUCKA = Person(
    name: 'Łucja Prabucka',
...
const Person LUKAS_JANOSIS = Person(
    name: 'Lukas Janonis',
    org: Org.zhpNL,
    email: []
);
...
const Person MAGDALENA_BAJER = Person(
    name: 'Madzia',
    org: Org.zhp,
...
const Person MAKSYMILIAN_WERAN = Person(
...
const Person MAJA_GUCIK = Person(
    name: 'Maja gucik',
...
const Person MAJA_HAJDACKA = Person(
    name: 'Maja Hajdacka',
...
const Person MALWINA_TRUSZKOWSKA = Person(
...
const Person MARTA_GOLEBIOWSKA = Person(
...
const Person NATALIA_SZYMANIAK = Person(
...
const Person PATRYCJA_DUDZINSKA = Person(
...
const Person PATRYK_CZUPIK = Person(
    ...
    rankHarc: RankHarc.zhpHOc,
    org: Org.zhp,
    email: ["patryk.czupik@zhp.net.pl"]
);
...
const Person PAULA_PALA = Person(
    name: 'Paula Pala',
    hufiec: huf_zhp_ziemi_wadowickiej,
    rankHarc: RankHarc.dhd,
    org: Org.zhp,
...
const Person PIOTR_KRAKOWIAK = Person(
...
const Person PIOTR_SOSNOWSKI = Person(
...
const Person ROBERT_ROBOTYCKI = Person(
...
const Person SANDRA_RZESZUREK = Person(
    name: 'Sandra Rzeszutek',
    email: ['sandrarzeszutek@wp.pl']
);
const Person SARA_WALCZYNSKA_GORA = Person(
...
const Person SLAWOMIR_MILEWSKI = Person(
...
const Person STANISLAW_WRONSKI = Person(
    name: 'Stanisław Wroński'
);
const Person STEFAN_KRYCZKA = Person(
    name: 'Stefan Kryczka',
    rankHarc: RankHarc.zhpCwik,
...
const Person SZYMON_PADOK = Person(
    ...
    hufiec: huf_zhp_gorzow_wielkopolski,
    rankHarc: RankHarc.zhpCwik,
    org: Org.zhp,
...
const Person SZYMON_ZDZIEBKO = Person(
    name: 'Szymon Zdziebko'
);
const Person TADEUSZ_BOJANOWSKI = Person(
...
const Person TOMASZ_GORECKI = Person(
    name: 'Tomek Górecki',
...
const Person TOMASZ_SMOLKA = Person(
  name: 'Tomasz Smołka',
  org: Org.zhr,
);
...
const Person URSZULA_KOWALSKA = Person(
    name: 'Ula Kowalska',
...
const Person WERONIKA_KOLCZ = Person(
    name: 'Weronika Kołcz',
    rankInstr: RankInstr.pwd,
    org: Org.zhp,
...
const Person WIKTORIA_POPIS = Person(
    ...
    rankHarc: RankHarc.zhpHOd,
    org: Org.zhp,
    email: ["wiktoria.popis@zhp.net.pl"]
);
...
const Person WIKTORIA_WENCEL = Person(
    ...
    hufiec: 'Ostrzeszowski hufiec harcerek "Agat" im. szarych szeregów',
    rankHarc: RankHarc.zhrOchotniczka,
    org: Org.zhr,
...
const Person WINCENTY_DIETRYCH = Person(
...
const Person WOJCIECH_GRUSZCZYNSKI = Person(
...
const Person WOJCIECH_TURSKI = Person(
...
const Person ZBYSZEK_CHODAKOWSKI = Person(
