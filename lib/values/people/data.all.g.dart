// GENERATED CODE - DO NOT MODIFY BY HAND
// dart format width=80

// **************************************************************************
// PersonFieldGenerator
// **************************************************************************

// GENERATED CODE - DO NOT MODIFY BY HAND

import 'package:harcapp_core/values/people/data.dart';
import 'package:harcapp_core/values/people/models.dart';

const List<RegisteredContributor> allRegisteredPeople = [
  ABRAHAM_PRAGER,
  ADAM_DAWID,
  ADAM_DUDAK,
  ADAM_KOLADA,
  ADAM_SKLODOWSKI,
  ADAM_SOBIERAJ,
  ADAM_WIECZOREK,
  ADAM_WOLOWIEC,
  AGATA_KOPYTKO,
  AGATA_MAJEWSKA,
  AGATA_MONKO,
  AGATA_STALA,
  AGNIESZKA_DURSKA,
  AGNIESZKA_PIEKARSKA,
  AGNIESZKA_RADECKA_KUBICKA,
  AGNIESZKA_SKUBA,
  AGNIESZKA_TYLKO,
  ALAN_FILAS,
  ALEKSANDER_BIL,
  ALEKSANDER_CHRZEST,
  ALEKSANDER_GROSZAN,
  ALEKSANDER_PALKA,
  ALEKSANDER_WELYCZKO,
  ALEKSANDRA_ANTKIEWICZ,
  ALEKSANDRA_CHRUSTEK,
  ALEKSANDRA_CWYNAR,
  ALEKSANDRA_GALIJ,
  ALEKSANDRA_KLEJDYSZ,
  ALEKSANDRA_KOSTRZEWA,
  ALEKSANDRA_KOWALSKA,
  ALEKSANDRA_KOZUBAL,
  ALEKSANDRA_KWAPISZ,
  ALEKSANDRA_MISIAK,
  ALEKSANDRA_SZKLARCZYK,
  ALEKSANDRA_TIMM,
  ALEKSANDRA_TKOCZ,
  ALEKSANDRA_TOWALSKA,
  ALEKSANDRA_WOJCIECHOWSKA,
  ALIAKSANDR_LAPKOUSKI,
  ALICJA_DOBOSZ,
  ALICJA_JASINSKA,
  ALICJA_JEZNACKA,
  ALICJA_LECIAK,
  ALICJA_MAJCHER,
  ALICJA_NOWOSAD,
  ALICJA_POROS,
  AMELIA_BOJARSKA,
  AMELIA_KALICKA,
  AMELIA_SITNICKA,
  AMELIA_SZALEWICZ,
  ANASTASIA_RESHCHENKO,
  ANGELIKA_MOSKAL,
  ANNA_RAJEWSKA,
  ANNA_SZMAJ,
  ANTONINA_CHALINSKA,
  ANTONINA_KARMANSKA,
  ANTONINA_PIERZCHALA,
  ANTONINA_ROMANSKA,
  ANTONINA_SZTYGLIC,
  ANTONI_ATANASSOW,
  ANTONI_CHMIELEWSKI,
  ANTONI_DEBICKI,
  ANTONI_KOLARCZYK,
  ANTONI_KOSZNIEC,
  ANTONI_RAK,
  ARTUR_KOSTRZEWA,
  ARTUR_RUSA,
  ASIA_P,
  BARBARA_THOMAS,
  BARTLOMIEJ_DANIELAK,
  BARTLOMIEJ_JASKOLSKI,
  BARTLOMIEJ_MALYJUREK,
  BARTLOMIEJ_SIUSTA,
  BARTOSZ_DABROWSKI,
  BARTOSZ_GORECKI,
  BARTOSZ_IGNASIAK,
  BARTOSZ_KIERES,
  BARTOSZ_KISIELEWSKI,
  BARTOSZ_KSIAZEK,
  BEBENEK_MIKOLAJ,
  BENIAMIN_PLUCINSKI,
  BLANKA_KARCZEWSKA,
  BLAZEJ_INGARDEN,
  BLAZEJ_KLEBBA,
  BLAZEJ_PLEWKA,
  BRUNO_BRONCEL,
  BRUNO_WALERYCH,
  CELINA_STANISLAWSKA,
  DAMIAN_SZYSZKA,
  DANIEL_IWANICKI,
  DANIEL_KORZEB,
  DANIEL_KRYSIAK,
  DARIUSZ_DYMEK,
  DAWID_KOBEDZA,
  DAWID_LOBODA,
  DAWID_PIOTR_JACHIMOWSKI,
  DAWID_WYSZYNSKI,
  DOMINIKA_GORZYNSKA,
  DOMINIKA_HOCHMANSKA,
  DOMINIKA_KONARSKA,
  DOMINIK_BETKA,
  DOMINIK_KIERES,
  DOMINIK_PRZYBYLOWSKI,
  DORIAN_JAREK,
  ECHO_TYL,
  EFA_BIEDNIAKOWA,
  EMILIA_ADAMCZYK,
  EMILIA_BARABASZ,
  EMILIA_WARSZAJLO,
  EMILIA_WITKOWSKA,
  EMILIA_Z_FSE,
  ERYK_UZNANSKI,
  EVELINA_STASILOVIC,
  EWA_BYSTRZYNSKA,
  EWELINA_HUPKA,
  FEARNE_MILEK,
  FILIP_BERGIER,
  FILIP_BRASZEWSKI,
  FILIP_JASTRZEBSKI,
  FILIP_KWIATKOWSKI,
  FILIP_LEWICKI,
  FILIP_PIELA,
  FILIP_SOCHAJ,
  FILIP_SWIDEREK,
  FLORIAN_PELEC,
  FRANCISZEK_FALENTA,
  FRANCISZEK_HALUCH,
  FRANCISZEK_LINKE,
  FRANCISZEK_MICHALSKI,
  FRANCISZEK_PUKNEL,
  FRANCISZEK_TOMASZCZYK,
  FRANCISZEK_WOJDYLO,
  GABRIELA_OZOG,
  GABRIELA_SAWICKA,
  GABRIELA_SOBCZAK,
  GABRIELA_TWORKOWSKA,
  GABRIEL_KOSSAKOWSKI,
  GABRIEL_POZNANSKI,
  GABRIEL_SIOREK,
  GAJA_TURECKA,
  GRZEGORZ_BOROWIEC,
  GRZEGORZ_FRANK,
  GRZEGORZ_FRANKOW,
  GRZEGORZ_GIBADLO,
  GRZEGORZ_PAWLAK,
  GRZEGORZ_ZAWADZKI,
  GUSTAW_BAJCAR,
  HANNA_BAJER,
  HANNA_CISAKOWSKA,
  HANNA_CZAJKOWSKA,
  HANNA_ERMAN,
  HANNA_KESKA,
  HANNA_KUCINSKA,
  HANNA_KUJAWSKA,
  HANNA_OLESZOWSKA,
  HANNA_RYBACKA,
  HANNA_STRZESZEWSKA,
  HANNA_WITKOWSKA,
  HANNA_WNEKOWICZ,
  HELENA_LATOSINSKA,
  HELENA_SWIATKOWSKA,
  HUBERT_CISLAK,
  HUBERT_FRUKOWSKI,
  HUBERT_JANIK,
  HUBERT_MILEROWICZ,
  HUBERT_NAPIERALA,
  HUBERT_SULSKI,
  HUGO_HANUSA,
  HUGO_ZASACKI,
  IDA_BECHTOLD,
  IGNACY_DUDZIAK,
  IGNACY_KOLTUNIAK,
  IGNACY_LESZCZYNSKI,
  IGNACY_SZYMICHOWSKI,
  IGNACY_WOJTCZAK,
  IGOR_KASPERSKI,
  IGOR_SOLECKI,
  INGA_WIERZBICKA,
  IZABELA_MOSKAL,
  JACEK_ANTKIEWICZ,
  JACEK_PELCZAR,
  JADWIGA_BILINSKA,
  JADWIGA_GORSKA,
  JADWIGA_SZEWCZYK,
  JAGIENKA_KULCZYCKA,
  JAGODA_BLASZCZYK,
  JAGODA_SKOWRONSKA,
  JAKUB_BRYLA,
  JAKUB_DEBICKI,
  JAKUB_DUDA,
  JAKUB_EJDUK,
  JAKUB_GABIS,
  JAKUB_HURYSZ,
  JAKUB_KLEPACZ,
  JAKUB_KLUCZKOWSKI,
  JAKUB_KRUCZKOWSKI,
  JAKUB_KUBICKI,
  JAKUB_LYSZKOWSKI,
  JAKUB_MAGIERA,
  JAKUB_MICHALSKI,
  JAKUB_MLYNSKI,
  JAKUB_NOGA,
  JAKUB_NOWAKOWSKI,
  JAKUB_SIKORA,
  JAKUB_SKUCHA,
  JAKUB_STEFANSKI,
  JAKUB_STRACZYNSKI,
  JAKUB_SWIT,
  JAKUB_TARNOWSKI,
  JAKUB_ZDANOWICZ_ZASIDKO,
  JANUSZ_ORLUTA,
  JAN_JARECKI,
  JAN_JAWORSKI,
  JAN_KRASZEWSKI,
  JAN_KUCZA,
  JAN_KWIATKOWSKI,
  JAN_LEWANDOWSKI,
  JAN_LICZBANSKI,
  JAN_MYSZCZYSZYN,
  JAN_NOWAK,
  JAN_REWERSKI,
  JAN_STANULA,
  JAROMIR_JABLONSKI,
  JAROSLAW_JAKUBIAK,
  JAROSLAW_ZASACKI,
  JASMINA_ROZYCKA,
  JERZY_ZOLNA,
  JOANNA_ASZKLAR,
  JOANNA_MICHALOWSKA,
  JOANNA_PAJAK,
  JOANNA_RACZKO,
  JOANNA_WALENDZIK,
  JOANNA_ZUBEK,
  JOLA_RYS,
  JOWITA_BUCZYNSKA,
  JULIANNA_KLUS,
  JULIAN_SLAZYK,
  JULIA_BENEDYK,
  JULIA_BIENIEK,
  JULIA_BOLOZ,
  JULIA_GRODZKA,
  JULIA_JAROSZ,
  JULIA_KARAS,
  JULIA_KOSZTYLA,
  JULIA_MACIAG,
  JULIA_MARCINIAK,
  JULIA_PIASKOWSKA,
  JULIA_PILCH,
  JULIA_PROSZKIEWICZ,
  JULIA_SIUDMAK,
  JULIA_SLAZAK,
  JULIA_SZOZDA,
  JULIA_TYSZKIEWICZ,
  JULIA_WIERZBA,
  JULIA_WIESZOLEK,
  JULITA_STEPIEN,
  KACPER_BACZKOWSKI,
  KACPER_CIESIELSKI,
  KACPER_FRONC,
  KACPER_JASINSKI,
  KACPER_KORDEK,
  KACPER_KOTECKI,
  KACPER_KOZLUK,
  KACPER_MIESOWICZ,
  KACPER_OLEJNIK,
  KACPER_SMOLKA,
  KACPER_SWITKIEWICZ,
  KACPER_SZCZENSY,
  KACPER_SZYMANKIEWICZ,
  KACPER_TOMCZYK,
  KACPER_TRUCHLEWSKI,
  KACPER_WIDZ,
  KACPER_WIETRZYKOWSKI,
  KAJETAN_MEDYK,
  KAJETAN_RUSZKOWSKI,
  KAJETAN_WITKOWSKI,
  KAJETAN_WYGNANSKI,
  KALINA_KAWICZ,
  KAMILA_GAJEWSKA,
  KAMIL_GORNIK,
  KAMIL_ORGANISTA,
  KAMIL_ZAK,
  KAROLINA_BABOL,
  KAROLINA_CZARNECKA,
  KAROLINA_HAJDUK,
  KAROLINA_MARCINKOWSKA,
  KAROLINA_MROCZKO,
  KAROLINA_WISNIEWSKA,
  KAROL_FRANKOWSKI,
  KAROL_FROST,
  KAROL_GOLABEK,
  KAROL_MALINSKI,
  KAROL_MALUS,
  KAROL_PODOLSKI,
  KATARZYNA_BIALAS,
  KATARZYNA_JEDRZEJCZYK,
  KATARZYNA_LISAK,
  KATARZYNA_MAZUR,
  KATARZYNA_POLANSKA_WILK,
  KATARZYNA_STUDNICKA,
  KATARZYNA_TRZESNIOWSKA,
  KINGA_BABIARSKA,
  KINGA_JANKO,
  KINGA_JASKULSKA,
  KINGA_KONOPCZYNSKA,
  KINGA_ZEBRACKA,
  KLARA_MAZEK,
  KLAUDIA_PARNIEWICZ,
  KLAUDIA_STASINSKA,
  KLAUDIUSZ_PALUCH,
  KLEMENTYNA_MARWICZ,
  KORDIAN_LATOCHA,
  KORNELIA_KASIBORSKA,
  KORNELIA_MROWKA,
  KORNELIA_PRZYCZOLKA,
  KORNEL_DABKOWSKI,
  KORNEL_GOLEBIEWSKI,
  KRYSTIAN_BULANDA,
  KRYSTYNA_BITNER,
  KRZESIMIR_KARBOWNIK,
  KRZYSZTOF_BANIK,
  KRZYSZTOF_GORECKI,
  KRZYSZTOF_KANIEWSKI,
  KRZYSZTOF_KRAWCZYK,
  KRZYSZTOF_LUBAS,
  KRZYSZTOF_MALIKIEWICZ,
  KRZYSZTOF_MALINOWSKI,
  KRZYSZTOF_PIOTR_WAGROWSKI,
  KRZYSZTOF_RODZINKA,
  KRZYSZTOF_SUCHARSKI,
  KSAWERY_TWORKOWSKI,
  KSENIA_OKRUCINSKA,
  LAURA_FRASZEWSKA,
  LAURA_NOWAKOWSKA,
  LENA_DOLECKA,
  LENA_PATLA,
  LENA_STEFANSKA,
  LENA_ULINSKA,
  LENA_WEISS,
  LILIANA_KASPRZYK,
  LILIANA_MATUSZYK,
  LILIANA_MIROTA,
  LUCIANO_COSTA,
  LUCJA_PRABUCKA,
  LUCJA_TALKOWSKA,
  LUKASZ_KRYWULT,
  LUKASZ_RYBINSKI,
  LUKASZ_STANISZEWSKI,
  LUKASZ_STROZYK,
  LUKASZ_SZEPIELAK,
  LUKASZ_SZTANDERA,
  LUKASZ_WERNIK,
  LUKAS_JANOSIS,
  MACIEJ_BATKO,
  MACIEJ_CHUSTECKI,
  MACIEJ_DOBROWOLSKI,
  MACIEJ_GRZELAZKA,
  MACIEJ_KOLAKOWSKI,
  MACIEJ_LADOS,
  MACIEJ_PAWLICA,
  MACIEJ_PRZYBYSZ,
  MACIEJ_SZOLC,
  MACIEJ_W,
  MACIEJ_WYSOCKI,
  MAGDALENA_BAJER,
  MAGDALENA_BINIAK,
  MAGDALENA_KALISZ,
  MAGDALENA_KOZLOWSKA,
  MAGDALENA_KROSZCZYNSKA,
  MAGDALENA_MIELNIK,
  MAJA_BUDZIOSZ,
  MAJA_GUCIK,
  MAJA_HAJDACKA,
  MAJA_SAWICKA,
  MAJA_SLOWINSKA,
  MAJA_WOJTYNIAK,
  MAJA_ZAJACZKOWSKA,
  MAKSYMILIAN_SPADLO,
  MAKSYMILIAN_TURZYNSKI,
  MAKSYMILIAN_WERAN,
  MAKSYM_KAWULA,
  MALGORZATA_KLOC,
  MALGORZATA_MASKO_HORYZA,
  MALGORZATA_ORANKIEWICZ,
  MALGORZATA_SZMUK,
  MALWINA_TRUSZKOWSKA,
  MARCELINA_SZYNDLER,
  MARCELINA_WILCZAK,
  MARCELI_WARDA,
  MARCEL_CHRZASCIK,
  MARCEL_MICHALIK,
  MARCEL_RYCHTER,
  MARCEL_WOZNIAK,
  MARCIN_JANKOWIAK,
  MARCIN_KOWALICKI,
  MARCIN_SOBKOWICZ,
  MARCJANNA_NEY,
  MAREK_BIZON,
  MAREK_BOJARUN,
  MAREK_LEWANCZYK,
  MAREK_MUSIALIK,
  MAREK_PRZYBYLA,
  MARIA_BATKO,
  MARIA_GRZYWACZ,
  MARIA_KIELIN,
  MARIA_LAKOMA,
  MARIA_MAGDALENA_DESKUR,
  MARIA_MIELCZAREK,
  MARIA_PIKOSZ,
  MARIA_PRZYBYLSKA,
  MARIA_SKOWRONEK,
  MARIA_STACHARSKA,
  MARLENA_BANIA,
  MARTA_GOLEBIOWSKA,
  MARTA_SZYMANDERSKA,
  MARTYNA_BULAKOWSKA,
  MARTYNA_CEGLAREK,
  MARTYNA_SADOWNIK,
  MARTYNA_WASILEWSKA,
  MARTYNA_ZAJAC,
  MARYSIA_SLUGAJ,
  MATEUSZ_CIAGLO,
  MATEUSZ_D,
  MATEUSZ_DYDO,
  MATEUSZ_GAWRYSIAK,
  MATEUSZ_KOBYLAREK,
  MATEUSZ_KORZENIOWSKI,
  MATEUSZ_MIKLASZEWSKI,
  MATEUSZ_OLSZANSKI,
  MATEUSZ_PYSZKA,
  MATEUSZ_STEPNIEWSKI,
  MATEUSZ_SWIEBODA,
  MATEUSZ_URBANIAK,
  MATEUSZ_WAS,
  MATVII_MASLOVSKYI,
  MATYLDA_OLEJNIK,
  MAXIMILIAN_STEINHOFF,
  MICHAL_CHOLEWCZYNSKI,
  MICHAL_DYDERSKI,
  MICHAL_JABCZYNSKI,
  MICHAL_JANAS,
  MICHAL_KARWOWSKI,
  MICHAL_KUSTOSIK,
  MICHAL_MACULEWICZ,
  MICHAL_METEL,
  MICHAL_PIENIAZEK,
  MICHAL_RZEZNIKIEWICZ,
  MICHAL_SITEK,
  MICHAL_SMULIK,
  MICHAL_SUPINSKI,
  MICHAL_ZAJAC,
  MIECZYSLAW_MICHALIK,
  MIESZKO_OKROJ,
  MIKOLAJ_GORECKI,
  MIKOLAJ_HORDEJUK,
  MIKOLAJ_LACHENDRO,
  MIKOLAJ_LUKASIK,
  MIKOLAJ_MATUSZEWSKI,
  MIKOLAJ_SOBON,
  MIKOLAJ_WITKOWSKI,
  MILENA_DULAK,
  MILENA_NOWICKA,
  MILOSZ_SZYMANSKI,
  MINION_MALGOSIA_MIKOLAJCZYK,
  NADIA_OSSOWSKA,
  NADIA_STOLAR,
  NADIA_WYSZOGRODZKA,
  NAPEWNO_NIEBRONIEWSKI,
  NATALIA_KLEPACKA,
  NATALIA_STODOLNA,
  NATALIA_STRZELECKA,
  NATALIA_SZYMANIAK,
  NATALIA_WOJTYCZKA,
  NATHAN_JULIAN_LEWKOWICZ,
  NATASZA_OLSZANSKA,
  NA_PEWNO_NIE_RYSZARD,
  NINA_GACEK,
  NINA_KESKA,
  NINA_MIKLAS,
  NINA_ROGUSKA,
  NORBERT_PIATKOWSKI,
  NORBERT_SOWA,
  OLAF_MILEROWICZ,
  OLEKSII_OVCHYNNIKOV,
  OLGA_JAJKO,
  OLGA_LUCZAK,
  OLIWIA_MAJDA,
  OLIWIA_STANCZYK,
  OLIWIER_STARCZEWSKI,
  OSKAR_PARDYAK,
  OSKAR_POLONSKI,
  OSKAR_SAKOWICZ,
  PATRYCJA_BINKIEWICZ,
  PATRYCJA_DUDZINSKA,
  PATRYCJA_KALINOWSKA,
  PATRYCJA_PIETRAS,
  PATRYCJA_POLOWCZYK,
  PATRYCJA_SPYRKA,
  PATRYCJA_SZCZESNA,
  PATRYCJA_TARCZYNSKA,
  PATRYCY_TUBACKI_BLASZCZYK,
  PATRYK_CZUPIK,
  PATRYK_GRABOWSKI,
  PATRYK_OLECH,
  PAULA_PALA,
  PAULINA_BURDZIK,
  PAULINA_FERENC,
  PAULINA_JASKULOWSKA,
  PAULINA_LUBOS,
  PAULINA_PODGORSKA,
  PAWEL,
  PAWEL_KIMEL,
  PAWEL_MARUD,
  PAWEL_SZCZYGIEL,
  PIOTR_BUKOWSKI,
  PIOTR_CHELMINIAK,
  PIOTR_GASIOR,
  PIOTR_KRAKOWIAK,
  PIOTR_KUBOWICZ,
  PIOTR_KWAPIEN,
  PIOTR_MACIEJ_KABATA,
  PIOTR_SOSNOWSKI,
  PIOTR_TUROWSKI,
  PIOTR_URBANIEC,
  PIOTR_ZIEMBIKIEWICZ,
  POLA_MARCINKOWSKA,
  PRZEMYSLAW_KLUCZKOWSKI,
  PRZEMYSLAW_KOWALIK,
  PRZEMYSLAW_MROCZKOWSKI,
  RADOSLAW_JASZCZAK,
  RADOSLAW_RELIDZYNSKI,
  RAFAL_ANTONICKI,
  RAFAL_BARAN,
  RAFAL_KOWALSKI,
  RAFAL_LALIK,
  RAFAL_RECZKIN,
  RAFAL_RZEPA,
  REMIGIUSZ_AJCHSZTET,
  ROBERT_LISZEWSKI,
  ROBERT_LOPATKA,
  ROBERT_MAZUR,
  ROBERT_ROBOTYCKI,
  RYSZARD_BIENIAS_6_TKDH_MROZY,
  RYSZARD_LASECKI,
  SAM_MAJA_PLOMINSKA,
  SANDRA_RZESZUREK,
  SARA_WALCZYNSKA_GORA,
  SEBASTIAN_BINKOWICZ,
  SEBASTIAN_KOPROWSKI,
  SEBASTIAN_SOBOLEWSKI,
  SEBASTIAN_SUGALSKI,
  SEWERYN_WOLINSKI,
  SLAWOMIR_MILEWSKI,
  SOFIJA_GALICKA,
  STANISLAW_KUDLA,
  STANISLAW_MARCHEWICZ,
  STANISLAW_WATOR,
  STANISLAW_WOJCIECHOWSKI,
  STANISLAW_WRONSKI,
  STEFA_RECZEK,
  STEFAN_KRYCZKA,
  STEFAN_PACIOREK,
  SZYMON_BARCZYK,
  SZYMON_BRODA,
  SZYMON_CHORAZY,
  SZYMON_DRATWINSKI,
  SZYMON_DROPEK,
  SZYMON_HARAZIM,
  SZYMON_JACKIEWICZ,
  SZYMON_JAWOREK,
  SZYMON_KLIMUNTOWSKI,
  SZYMON_LANDORF,
  SZYMON_MALCZAK,
  SZYMON_MASLOWSKI,
  SZYMON_OPLOCKI_NIEMIEC,
  SZYMON_OZOG,
  SZYMON_PADOK,
  SZYMON_PODGORNY,
  SZYMON_REKOWSKI,
  SZYMON_SITEK,
  SZYMON_TRZONKOWSKI,
  SZYMON_ZDZIEBKO,
  TADEUSZ_BOJANOWSKI,
  TADEUSZ_BRACHA,
  TAMANACO_NORIEGA,
  TOMASZ_BUKOWIECKI,
  TOMASZ_FLORCZAK,
  TOMASZ_GORECKI,
  TOMASZ_KOTOWSKI,
  TOMASZ_LUDWIG,
  TOMASZ_SMOLKA,
  TOMASZ_ZAGORSKI,
  TOMASZ_ZGORSKI,
  TOSIA_BANASZAK,
  TOSIA_KLEPACKA,
  TYMON_TALECKI,
  TYMOTEUSZ_JAWORSKI,
  TYTUS_LIPKA,
  URSZULA_KOWALSKA,
  VICTORIA_TROCZYNSKA,
  VIVIENNE_FALKOWSKA,
  WANDA_MARCHEL,
  WERONIKA_IWANISZYN,
  WERONIKA_KOLCZ,
  WERONIKA_MATECKA,
  WERONIKA_PUSCIAN,
  WERONIKA_WICHER,
  WERONIKA_ZAWIERUCHA,
  WIKTORIA_BECHERKA,
  WIKTORIA_DRGAS,
  WIKTORIA_LUKASIK,
  WIKTORIA_PINKOWSKA,
  WIKTORIA_POPIS,
  WIKTORIA_PRUSZYNSKA,
  WIKTORIA_WENCEL,
  WIKTORIA_WOJCIK,
  WIKTOR_KARPALA,
  WIKTOR_KOWALCZUK,
  WINCENTY_DIETRYCH,
  WITOLD_BASIURA,
  WITOLD_BRACHA,
  WITOLD_FIALKOWSKI,
  WITOLD_JAKUBOWSKI,
  WOJCIECH_BOSOWSKI,
  WOJCIECH_GODECKI,
  WOJCIECH_GOLIS,
  WOJCIECH_GRUSZCZYNSKI,
  WOJCIECH_JUCYK,
  WOJCIECH_KITA,
  WOJCIECH_KORZENIOWSKI,
  WOJCIECH_KUCHARSKI,
  WOJCIECH_PONIEWSKI,
  WOJCIECH_SOBOL,
  WOJCIECH_TURSKI,
  WOJCIECH_WALACH,
  WOJCIECH_WOLNIK,
  WOJCIECH_ZIELINSKI,
  WOJTEK_KRAWIEC,
  ZBYSZEK_CHODAKOWSKI,
  ZOFIA_FABROWSKA,
  ZOFIA_KOSIDER,
  ZOFIA_SZAFRANEK,
  ZOFIA_ZAWADZKA,
  ZOFIA_ZBRUK,
  ZUZANNA_ANDRZEJCZAK,
  ZUZANNA_BIALA,
  ZUZANNA_CHMIEL,
  ZUZANNA_DUDEK,
  ZUZANNA_DZIEDZIC,
  ZUZANNA_GRZESIAK,
  ZUZANNA_GUGALA,
  ZUZANNA_JANKOWSKA,
  ZUZANNA_JAWORSKA,
  ZUZANNA_KOLIS,
  ZUZANNA_KOWALCZYK,
  ZUZANNA_MIERZEJEWSKA,
  ZUZANNA_NAWROT,
  ZUZANNA_NIEWEGLOWSKA,
  ZUZANNA_PIWKO,
  ZUZANNA_RADKOWSKA,
  ZUZANNA_RELKOWSKA,
  ZUZANNA_ROMANISZYN,
  ZUZANNA_SZYSZKA,
  ZUZANNA_WARCHOL,
];
